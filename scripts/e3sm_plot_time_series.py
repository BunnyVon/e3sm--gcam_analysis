...
default_inputs = {
    ...
    'include_annual_mean_across_all_sets': False,
    'include_monthly_mean_across_all_sets': False,
...
def plot_time_series(inputs):
    ...
    if monthly_time_series_plot:
        ...
        # Update the x_scale to None so that month names appear on the x-axis, rather than the numerical values (month numbers).
        plot_options['x_scale'] = None
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    if len(sys.argv) < 2:
        print('Usage: python e3sm_plot_time_series.py `path/to/json/input/file(s)\'')
        sys.exit()
